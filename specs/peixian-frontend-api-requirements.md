...
| MODEL-03 | `PATCH /admin/models/{id}` | 编辑、启停、默认切换 | 现有可用 | P0 |
...
| USER-03 | `PATCH /admin/users/{id}` | 编辑、启停用户 | 现有可用，前端部分待接入 | P0 |
...
| USER-05 | `POST /admin/users/{id}/reset-password` | 重置密码 | 后端已有、前端待接入 | 按钮当前无事件 |
| USER-06 | `PATCH /admin/users/{id}` | 列表直接启用/禁用 | 后端已有、前端待接入 | 按钮当前无事件 |
...
关键对齐问题：当前前端直接把 `skill_ids/plugin_ids` 当名称展示，而后端返回真实 ID 时可读性很差。建议增加 `skills/plugins` 展示对象，前端改用 `name`。
...
7. 用户重置密码、直接启停、调用详情和筛选控件完成前端绑定。
8. 调用审计返回 Skill/插件显示名称，并增加分页和完整筛选。
...
- 模型分页、筛选、新增、编辑、测试、启停、默认切换正确。
- API Key 不回显，保存前测试不落库。
- 用户分页筛选、组织属性、启停、重置密码和一次性密码展示正确。
- 非空部门删除返回 409。
- 调用审计组合筛选、详情和导出一致，敏感信息不泄漏。
