...
const mockInvocations: Invocation[] = [
  ...
  id: `mock-invocation-${index}`, run_id: `mock-run-${index}`, session_id: `session-${String(index + 1).padStart(4, "0")}`, created: 1726160076 - index * 7200, username: user, display_name: user,
...
export default function FinalAdmin(props: { section: Section }) {
  ...
  const shownInvocations = createMemo(() => filter(invocations().length ? invocations() : mockInvocations, query(), (item) => `${item.display_name} ${item.department_name} ${item.query_summary} ${item.session_id}`))
  ...
  async function setModelEnabled(item: Model, enabled: boolean) {
    ...
    if (item.id.startsWith("mock-")) {
      setModels((models().length ? models() : mockModels).map((value) => value.id === item.id ? { ...value, enabled, test_status: enabled ? "enabled" : "disabled" } : value))
      app.notify(enabled ? "演示模型已启用。" : "演示模型已关停。")
      return
    }
  ...
  async function resetUserPassword(item: User) {
    if (item.id.startsWith("mock-")) return app.notify("演示账号已完成重置密码操作。")
    try { await post("/admin/users/" + item.id + "/reset-password"); app.notify("密码已重置，请通过安全渠道告知用户。") }
    catch (cause) { app.notify(safeMessage((cause as Error).message), "error") }
  }
  async function disableUser(item: User) {
    if (item.active === false) return
    if (item.id.startsWith("mock-")) {
      setUsers((users().length > 1 ? users() : mockUsers).map((value) => value.id === item.id ? { ...value, active: false } : value))
      app.notify("演示账号已禁用。")
      return
    }
  ...
  return <div class="police-admin-page">
    ...
    <Show when={props.section === "users"}><UsersPage users={shownUsers()} departments={shownDepartments()} summary={userSummary()} query={query()} setQuery={setQuery} departmentsMode={showDepartments()} setDepartmentsMode={setShowDepartments} editUser={setUserForm} editDepartment={setDepartmentForm} resetPassword={resetUserPassword} disableUser={disableUser} /></Show>
